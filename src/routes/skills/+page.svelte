...
<div class="relative z-10 min-h-screen flex flex-col justify-center items-center px-6 py-24 text-white">
	<div class="max-w-6xl w-full">
		...
		<div class="text-center mb-16">
			<h1
				class="text-4xl md:text-5xl font-bold mb-3 bg-gradient-to-r from-blue-400 to-cyan-400 text-transparent bg-clip-text flex items-center justify-center gap-3"
				in:fade={{ duration: 800 }}
			>
				<i class="fas fa-lightbulb text-white animate-pulse hover:animate-bounce cursor-pointer text-3xl md:text-4xl hover:text-yellow-300 transition-colors duration-300"></i>
				Skills
			</h1>
			<p class="text-neutral-400 text-lg max-w-2xl mx-auto" in:fade={{ delay: 200, duration: 600 }}>
				Transforming concepts into impactful digital solutions
			</p>
		</div>

		<!-- Timeline-style layout -->
		<div class="relative border-l border-neutral-800 ml-4 space-y-16">
			{#each Object.entries(skills) as [category, list], i}
				<div class="relative pl-8" in:fly={{ y: 40, duration: 600, delay: i * 200 }}>
					<!-- Timeline dot -->
					<div class="timeline-dot absolute left-[-7px] top-2 w-3 h-3 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-full"></div>

					<!-- Category heading -->
					<h2 class="text-2xl font-semibold mb-6 text-neutral-100">
						{category}
					</h2>
					...
					<div class="flex flex-wrap gap-3">
						{#each list as skill, j}
							<span
								...
								class="skill-tag px-4 py-2 rounded-full bg-gradient-to-r from-neutral-900/60 to-neutral-800/40 backdrop-blur-md border border-neutral-700 text-neutral-300 text-sm md:text-base transition-all duration-300 hover:scale-105 hover:border-blue-500/50 cursor-default relative overflow-hidden"
